...
class NewsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $file1        = $request->file('media');
        $filename1    = $file1->getClientOriginalName();

        $newsValue = [  "gttop"	    => $clubcode,
                        "title" 	  => $request->input('title'),
                        "desc"    	=> $request->input('desc'),
                        "date"    	=> $request->input('date'),
                        "reference"	=> $request->input('reference'),
                        "berita"    => $request->input('berita'),
                        // "tumbnail" 	=> $request->file('gambar'),
        ...
        $response = Curl::to('128.199.161.172:8108/addnews')
                    ...
                    ->withFile( 'tumbnail', '/var/www/media/'.$filename, 'image/jpg/png',$request->file('gambar'))
                    ->withFile( 'files', '/var/www/media/'.$filename1, 'image/jpg/png', $request->file('gambar'))
                    ->post();
